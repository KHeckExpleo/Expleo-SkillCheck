// script.js - The main logic of SkillCheck
...
import { CONFIG, loadExamData } from './Expleo_ESC_loader_configuration.js';
import { UICONFIG, formatText } from './Expleo_ESC_loader_UItext.js';
...
//        examData = await response.json();
		...
		const infoQuestionCountElement = document.getElementById('info-question-count');
		if (infoQuestionCountElement) infoQuestionCountElement.textContent = totalQuestions;
...
function updateUnansweredQuestions_old() {
...
// Platzhalter sicherstellen (ID, Text aus UICONFIG, Position 0)
function ensurePlaceholder(select) {
  // Bevorzugt aus deiner UI-Text-Konfiguration:
  // - erst dropdowns.yourChoice
  // - dann labels.yourChoice
  // - sonst Fallback
  const txt =
    (UICONFIG?.dropdowns?.yourChoice ??
     UICONFIG?.labels?.yourChoice ??
     '– Frage auswählen –').toString().trim();

  // vorhandene Placeholder-Option finden oder erzeugen
  let ph =
    select.querySelector('option#yourChoice') ||
  ...
  if (!ph) {
    ...
    ph.value = '';
    ph.selected = true;
    ph.disabled = true; // auswählbar? -> auf false setzen
    select.prepend(ph);
  }
  ...
  // sicherstellen, dass Placeholder an Position 0 bleibt
  if (select.firstElementChild !== ph) {
    select.prepend(ph);
  }
}


// He: 19.12.2025 – NEU: Dropdown statt Liste
function updateUnansweredQuestions() {
  ...
  const container= document.getElementById('unanswered-container');

  if (!select) {
    console.warn('[updateUnansweredQuestions] #unanswered-select nicht gefunden.');
    if (container) container.style.display = 'none';
    return;
  }

  // alle Fragen im DOM ermitteln
  const questionElements = document.querySelectorAll('.question[id^="question-"]');
  const unansweredIndices = [];
  const questionWord = UICONFIG?.labels?.questionWord ?? 'TBD';

  questionElements.forEach(qEl => {
    const num = qEl.id.replace('question-', ''); // "1","2",...
    const qName = `q${num}`;                     // "q1","q2",...
    if (userAnswers[qName] === undefined) {
      unansweredIndices.push(Number.parseInt(num, 10)); // Zahlen-Index sammeln
    }
  });

  // Sichtbarkeit steuern
  if (container) {
    container.style.display = unansweredIndices.length > 0 ? 'block' : 'none';
  }

  // ❌ ALT: überschreibt dynamische Texte und entfernt IDs
  // select.innerHTML = '<option value="">– Frage auswählen –</option>';

  // ✅ NEU: sicher leeren + Platzhalter programmatisch setzen
  select.textContent = '';
  ensurePlaceholder(select); // Text aus UICONFIG, id="yourChoice", selected/disabled

  // Optionen anhängen
  for (const num of unansweredIndices) {
    const opt = document.createElement('option');
    opt.value = String(num - 1);             // 0-basierter Index für showQuestion(n)
    opt.textContent = `${questionWord} ${num}`; // Anzeige 1-basiert
    select.appendChild(opt);
  }

  // Zähler aktualisieren (optional)
  if (countEl) countEl.textContent = `(${unansweredIndices.length})`;
}
...
function initApp() {
	showQuestion(currentQuestion);
	updateCurrentScore();
	queueMicrotask(() => updateUnansweredQuestions());
	initLOFilter();
...
// Einmalig den Change-Handler für das Dropdown registrieren
document.addEventListener('DOMContentLoaded', () => {
  ...
  select.addEventListener('change', (e) => {
    const zeroBased = Number.parseInt(e.target.value, 10);
    if (!Number.isInteger(zeroBased) || zeroBased < 0) return;

    // 0-basierten Wert auf 1-basiert transformieren
    currentQuestion = zeroBased + 1;
    showQuestion(currentQuestion);
    updateNavigationButtons();
    e.target.selectedIndex = 0; // UX: zurück auf Platzhalter
  });
});
