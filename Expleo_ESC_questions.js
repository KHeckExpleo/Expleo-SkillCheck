...
import { UICONFIG } from './Expleo_ESC_loader_UItext.js';
...
export function renderQuestions(data) {
  ...
  data.forEach(item => {
    if (!item?.Details) return;
	  // LO-Code von der äußeren Ebene holen:
	  const loCode = String(item.LearningObjective ?? '').trim();
      item.Details.forEach(detail => {
	    ...
	    const answerOptionRaw = detail.AnswerOption;
	    const hasAnswerOption = (answerOptionRaw !== undefined && answerOptionRaw !== null && String(answerOptionRaw).trim() !== '');
	    const answerOptionLabel = hasAnswerOption ? String(answerOptionRaw).trim() : '';
	  ...
	  // Optional: LO auch als Datenattribut ablegen (später nützlich fürs Filtern)
	  if (loCode) questionDiv.dataset.lo = loCode;
	  ...
	  // Reihenfolge: Nummer → LO-Badge → AnswerOption-Badge
	  const loBadgeHTML = loCode ? `<span class="lo-badge"> - LO: ${loCode}</span>` : '';
	  // NEU: Inline-Label neben der Nummer einbauen (nur, wenn vorhanden)
	  const optionBadgeHTML = hasAnswerOption
	    ? `<span class="answer-option-badge">– ${answerOptionLabel}</span>`
	    : '';

		titleEl.innerHTML = `
		  <div class="question-number">
		    ${questionWord} ${numericId}${loBadgeHTML} ${optionBadgeHTML}
		  </div>
		  <div class="question-text">${questionText}</div>
		`;
...
export function updateUnansweredQuestions_kglw() {
